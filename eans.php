...
$conn=new mysqli($dbservername,$dbusername,$dbpassword,$dbname);
  ...
  $query="SELECT user.username, answer.ans, answer.aid FROM user, answer WHERE user.id=answer.ida";
  ...
  if ($result->num_rows > 0)
   {
    ...
    {
        $id=$row['aid'];
      echo "<tr>"."<td>".$sr."</td>"."<td>".$row["username"]."</td>"."<td>".$row["ans"]."</td>"."<td>"."<form method='POST' action='dans.php'><input type='hidden' name='id' value='$id'><input type='submit' value='Delete Answer'></form>"."</td>"."</tr>";
      $sr++;
    }
  }
  else
    {
      echo "<h5>No Answers yet.</h5>";
    }
}
